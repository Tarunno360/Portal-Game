...
yaw_speed = 2.0
# Bullet data
bullets = []  # Each bullet: {'pos': [x, y, z], 'dir': [dx, dy, dz]}
bullet_speed = 3.2
...
def shoot_bullet():
    ...
    # Start position: player_pos + direction * small offset (barrel tip)
    start_pos = [
        player_pos[0] + lx * 0.7,
        player_pos[1] + ly * 0.7,
        player_pos[2] + lz * 0.7
    ...
    direction = [lx, ly, lz]
    bullets.append({'pos': start_pos, 'dir': direction})

def update_bullets(dt):
    remove_list = []
    for bullet in bullets:
        bullet['pos'][0] += bullet['dir'][0] * bullet_speed * dt
        bullet['pos'][1] += bullet['dir'][1] * bullet_speed * dt
        bullet['pos'][2] += bullet['dir'][2] * bullet_speed * dt
        # Remove if too far
        if (abs(bullet['pos'][0] - player_pos[0]) > 50 or
            abs(bullet['pos'][1] - player_pos[1]) > 50 or
            abs(bullet['pos'][2] - player_pos[2]) > 50):
            remove_list.append(bullet)
    for b in remove_list:
        bullets.remove(b)

def draw_bullets():
    glColor3f(0.2, 0.7, 1.0)  # Portal blue
    for bullet in bullets:
        glPushMatrix()
...
def boundary_player_position():
    # Room boundaries (adjust to match actual wall positions)
    min_x, max_x = 0.5, 59.5
    min_z, max_z = 0.5, 11.5
...
# dimensions: 60 (length) x 12 (width)
    
    # Back wall
    create_wall_with_tiles((0, 0), (60, 0), rows=2, cols=13)
    
    # Front wall
    create_wall_with_tiles((0, 12), (60, 12), rows=2, cols=13)

    # Left wall
    create_wall_with_tiles((0, 0), (0, 12), rows=2, cols=6)

    # Right wall with door 
    create_wall_with_tiles_with_door((60, 0), (60, 12), rows=2, cols=6, door_rows=[0, 1], door_col=3)
...
def main():
    ...
    glutMouseFunc(mouse_click)  # Right mouse click handler
    glutIdleFunc(idle)
