...
def reset_game():
    ...
    global door_color, button_activated, v_y, is_falling, last_teleport_time, wall_tiles
    ...
    is_falling = False
    last_teleport_time = 0.0
    print("Game reset: Player at [10.0, 0.0, 10.0], tiles gray, door red, button off")

def distance_to_line_segment(p, a, b):
    """Compute the shortest distance from point p to line segment ab in 2D (x, z)."""
    px, pz = p[0], p[2]
    ax, az = a[0], a[2]
    bx, bz = b[0], b[2]
    # Vector from a to b
...
def check_button_laser_collision():
    """Check if player is too close to button laser lines, reset game if so."""
    global player_pos
    # Check if player is at laser height
    if not (0.0 - 0.5 <= player_pos[1] <= BUTTON_HEIGHT + 0.5):
        return
    # Laser lines at y=0.833
    # Line 1: (8.5, 0.833, 0.5) to (9.0, 0.833, 8.5)
    # Line 2: (9.0, 0.833, 8.5) to (0.0, 0.833, 8.5)
    line1_a = [8.5, 0.833, 0.5]
    line1_b = [9.0, 0.833, 8.5]
    line2_a = [9.0, 0.833, 8.5]
    line2_b = [0.0, 0.833, 8.5]
    # Compute distances to each line segment
    dist1 = distance_to_line_segment(player_pos, line1_a, line1_b)
    dist2 = distance_to_line_segment(player_pos, line2_a, line2_b)
    # Player radius ~0.5
    if dist1 <= 0.5 or dist2 <= 0.5:
        print(f"Player at {player_pos} too close to button laser (dist1={dist1:.2f}, dist2={dist2:.2f}), resetting game")
        reset_game()

def check_door_collision():
    """Check if player touches door or door laser, reset game if so."""
    global player_pos
    # Door/laser bounds: x=0 to 20, y=0 to 6, z ≥ 14.5
    if (0.0 - 0.5 <= player_pos[0] <= 20.0 + 0.5 and
        0.0 - 0.5 <= player_pos[1] <= 6.0 + 0.5 and
        player_pos[2] >= 14.5):
        print(f"Player at {player_pos} touched door/laser at x=0-20, y=0-6, z≥14.5, resetting game")
        reset_game()
...
def keyboard(key, x, y):
    global player_pos, bullets, wall_tiles, blue_shot_fired, yellow_shot_fired, mouse_captured
...
def draw_button():
    """Draw a vertical cylindrical button at BUTTON_POS with lower circular face on floor and L-shape laser."""
    glPushMatrix()
    glTranslatef(BUTTON_POS[0], BUTTON_POS[1], BUTTON_POS[2])
    # Rotate cylinder so it aligns with Y-axis (standing up from y=0)
    glRotatef(-90, 1, 0, 0)
    glColor3f(0.1, 0.2, 0.3) 
    glEnable(GL_DEPTH_TEST)
    quad = gluNewQuadric()
    # Draw cylinder from z=0 to z=BUTTON_HEIGHT (now mapped to y-axis)
    gluCylinder(quad, BUTTON_RADIUS, BUTTON_RADIUS, BUTTON_HEIGHT, 32, 8)
    # Draw bottom disk at z=0 (which is now y=0)
    gluDisk(quad, 0.0, BUTTON_RADIUS, 32, 8)
    # Move to top of cylinder (z = BUTTON_HEIGHT → y = BUTTON_HEIGHT)
    glTranslatef(0.0, 0.0, BUTTON_HEIGHT)
    gluDisk(quad, 0.0, BUTTON_RADIUS, 32, 8)
    glPopMatrix()

    # Draw red L-shape laser at y=BUTTON_HEIGHT
    glPushMatrix()
    glColor3f(1.0, 0.0, 0.0)
    glBegin(GL_LINES)
    # Line 1: (8.5, 0.833, 0.5) to (9.0, 0.833, 8.5)
    glVertex3f(8.5, BUTTON_HEIGHT, 0.5)
    glVertex3f(9.0, BUTTON_HEIGHT, 8.5)
    # Line 2: (9.0, 0.833, 8.5) to (0.0, 0.833, 8.5)
    glVertex3f(9.0, BUTTON_HEIGHT, 8.5)
    glVertex3f(0.0, BUTTON_HEIGHT, 8.5)
    glEnd()
    glPopMatrix()
    print(f"Drawing button at {BUTTON_POS}, height={BUTTON_HEIGHT}, radius={BUTTON_RADIUS}, L-shape laser at y={BUTTON_HEIGHT}")

def draw_laser_door():
    """Draw a red quadrilateral laser across the entire front wall."""
    glPushMatrix()
    glColor3f(1.0, 0.0, 0.0)
    glBegin(GL_LINE_LOOP)
    glVertex3f(0.0, 0.0, 15.0)   # Bottom-left
    glVertex3f(20.0, 0.0, 15.0)  # Bottom-right
    glVertex3f(20.0, 6.0, 15.0)  # Top-right
    glVertex3f(0.0, 6.0, 15.0)   # Top-left
    glEnd()
    glPopMatrix()
    print("Drawing door laser quadrilateral at x=0-20, y=0-6, z=15.0")
...
def check_button_interaction():
    ...
    if button_activated:
        return  # Door color already set to green
    # Calculate distance on XZ-plane
    dx = player_pos[0] - BUTTON_POS[0]
    dz = player_pos[2] - BUTTON_POS[2]
    distance = math.sqrt(dx**2 + dz**2)
    # Check if player is within button radius and close to button height
    if distance <= BUTTON_RADIUS and player_pos[1] <= BUTTON_HEIGHT + 0.1:
        door_color = 'green'
        button_activated = True
        print(f"Player at {player_pos} stepped on button: Door color set to green permanently")
    else:
        print(f"Player at {player_pos}, button not activated yet")

def draw_tile_with_door_color(tile):
    """Wrap draw_tile to apply door color to door tiles (cols 4-8, rows 0-1)."""
    coords, color = tile
    # Identify door tiles (front wall, row=0 or 1, cols 4-8)
    x_min = min(v[0] for v in coords)
    x_max = max(v[0] for v in coords)
    y_min = min(v[1] for v in coords)
    z_min = min(v[2] for v in coords)
    # Front wall (z ≈ 20), rows 0 or 1 (y_min < 6), cols 4-8 (x ≈ 6.67 to 13.33)
    is_door_tile = (abs(z_min - room_size) < 0.1 and  # Front wall (z=20)
                    y_min < 6.0 and  # Rows 0 or 1 (y=0 to 6)
                    6.67 - 0.1 <= x_min <= 13.33 + 0.1)  # Cols 4-8
    if is_door_tile:
        print(f"Door tile detected at x_min={x_min:.2f}, y_min={y_min:.2f}, z_min={z_min:.2f}, applying {door_color}")
        # Draw the tile with door_color
        glEnable(GL_POLYGON_OFFSET_FILL)
        glPolygonOffset(1.0, 1.0)  # Slightly less offset to ensure visibility
...
def display():
    ...
    check_door_collision()  # Check door/laser collision
    check_button_laser_collision()  # Check button laser collision
    check_button_interaction()  # Check button state
    print(f"Player position: {player_pos}, is_falling={is_falling}, door_color={door_color}, button_activated={button_activated}")
    lx = math.sin(math.radians(player_yaw)) * math.cos(math.radians(player_pitch))
    ly = math.sin(math.radians(player_pitch))
    lz = -math.cos(math.radians(player_yaw)) * math.cos(math.radians(player_pitch))
    ...
    draw_button()  # Draw button with laser
    draw_laser_door()  # Draw door laser
    for tile in wall_tiles:
        draw_tile_with_door_color(tile)  # Draw tiles
    for bullet in bullets:
        draw_bullet(bullet)
    glLoadIdentity()
    glDisable(GL_DEPTH_TEST)
