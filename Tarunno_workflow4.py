...
def draw_button():
    """Draw a vertical cylindrical button at BUTTON_POS with lower circular face on floor."""
    glPushMatrix()
    ...
    gluDisk(quad, 0.0, BUTTON_RADIUS, 32, 8)
    glPopMatrix()
    print(f"Drawing vertical button at {BUTTON_POS}, height={BUTTON_HEIGHT}, radius={BUTTON_RADIUS}")
...
def display():
    ...
    for tile in wall_tiles:
        draw_tile_with_door_color(tile)  # Use new function for tiles
    for bullet in bullets:
        draw_bullet(bullet)
    glLoadIdentity()
    glDisable(GL_DEPTH_TEST)
